   ...
   <body>
     ...
     <div id="page-wrapper">
			 <div class="container-fluid text-center" style="max-width:700px;">
					...
					<h3>Customer Loyalty Trajectory</h3>
					<div class="chart b-chart">

					</div>
					<!--h3> Customer Purchase by Genre</h3>
     ...
     <script type="text/javascript">
       ...
       $(function() {
				 ...
				 <?php
					...
					$res = json_encode($_SESSION['loyalty']);
         ...
         var x = chart.addCategoryAxis("x", "Month"); // <- change here if any changes to the field names
         chart.addMeasureAxis("y", "Value"); // <- change here if any changes to the field names
         chart.addSeries("Purchase", dimple.plot.line);
         x.addOrderRule(["Jan","Feb","Mar","Apr","May","Jun","July","Aug","Sep","Oct","Nov","Dec"]);
         chart.addLegend(60,20,500,20, "right");
         chart.draw();

             var svg2 = dimple.newSvg(".b-chart", 700, 700);
             var datasetb = <?php echo json_encode($res_arr[1])  ?>

     		 // Create and Position a Chart
     		 var chartb = new dimple.chart(svg2, datasetb);
      		var x = chartb.addCategoryAxis("x", "Month");
      		...
      		chartb.addColorAxis("Loyalty", ["red", "yellow", "green"]);

      		// Add a thick line with markers
      		chartb.addSeries("ID", dimple.plot.line); 
      		chartb.addLegend(60,20,500,20,"right")
